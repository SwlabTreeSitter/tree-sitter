...
# 1. 실행 파일 및 라이브러리 경로 (Ruby 언어용)
EXE_PATH = "/home/hyeonjin/PL/tree-sitter/TreeSitterCutFile.exe"
LIB_PATH = "/home/hyeonjin/PL/tree-sitter-ruby/ruby.so"

# 2. 데이터셋 및 정답지 경로
SOURCE_DIR = "/home/hyeonjin/PL/codecompletion_benchmarks/ruby/TEST"
ANSWER_DIR = "/home/hyeonjin/PL/tree-sitter/reports/ruby"
REPORT_DIR = "/home/hyeonjin/PL/tree-sitter/reports/ruby"

# 3. DB 경로 (구조 후보 추천용)
DB_PATH = "/home/hyeonjin/PL/code-completion-extension/resources/ruby/candidates.json"
...
class FileReporter:
    def __init__(self):
        ...
        if not os.path.exists(REPORT_DIR):
            ...
            for csv_file in ["ruby_file_performance.csv", "ruby_rank_distribution.csv"]:
    ...
    def save_debug_log(self, safe_name, log_data):
        debug_dir = os.path.join(REPORT_DIR, "debug_states_ruby")
    ...
    def save_file_performance_report(self):
        csv_path = os.path.join(REPORT_DIR, "ruby_file_performance.csv")
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
    ...
    def run(self):
        ...
        print(f"[*] Found {len(target_files)} '.rb' files. Starting Iterative Analysis...")
