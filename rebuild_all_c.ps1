...
$LANG = Join-Path $ROOT "tree-sitter-c"
...
Assert-Path $LANG "tree-sitter-c repo"
...
Write-Host "=== [2] generate/build (tree-sitter-c) ==="
Push-Location $LANG
...
if (-not $SkipParse) {
  ...
  $sample = Join-Path $LANG "C_Sample\hello_world.c"
  if (Test-Path $sample) {
    Push-Location $LANG
