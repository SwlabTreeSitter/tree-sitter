...
echo ""
echo "=== [2] Generate/Build (tree-sitter-cpp) ==="
cd "$LANG_DIR"
...
# grammar.js가 tree-sitter-c/grammar를 require하므로 tree-sitter-c JS 파일만 필요
# --ignore-scripts: node-gyp 바인딩 빌드는 불필요하므로 스킵
echo " -> Installing npm dependencies (tree-sitter-c)..."
npm install --ignore-scripts

TOKEN_MAP_OUTPUT_DIR="$EXT_DIR/resources/cpp" "$TS_CLI" generate
"$TS_CLI" build

echo ""
echo "=== [3] Optional Parse Smoke Test ==="
SAMPLE_TMP="$TS_DIR/_smoke_test_cpp.cpp"
cat > "$SAMPLE_TMP" << 'EOF'
#include <iostream>
...
if [ -f "to_data_batch_collect_learn_cpp.py" ]; then
    python3 to_data_batch_collect_learn_cpp.py
    python3 to_json_aggregate_cpp.py
else
    echo " -> (skip) to_data_batch_collect_learn_cpp.py not found"
fi
...
if [ -d "$EXT_DIR" ]; then
    ...
    NODE_OUT="build/Release/cpp_parser_addon.node"
    if [ -f "$NODE_OUT" ]; then
