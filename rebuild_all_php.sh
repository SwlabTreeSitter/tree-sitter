...
LANG_DIR="$ROOT/tree-sitter-php"
GRAMMAR_DIR="$LANG_DIR/php"          # php (<?php 태그 포함 전체 문법) 사용
...
echo ""
echo "=== [2] Generate/Build (tree-sitter-php) ==="
cd "$GRAMMAR_DIR"
...
TOKEN_MAP_OUTPUT_DIR="$EXT_DIR/resources/php" "$TS_CLI" generate
"$TS_CLI" build

if [ ! -f "$GRAMMAR_DIR/parser.so" ]; then
    echo "Error: parser.so not found after build"
    exit 1
fi
echo " -> Built: $GRAMMAR_DIR/parser.so"

echo ""
echo "=== [3] Optional Parse Smoke Test ==="
SAMPLE_TMP="$TS_DIR/_smoke_test_php.php"
cat > "$SAMPLE_TMP" << 'EOF'
<?php
...
if [ -f "to_data_batch_collect_learn_php.py" ]; then
    python3 to_data_batch_collect_learn_php.py
    python3 to_json_aggregate_php.py
else
    echo " -> (skip) to_data_batch_collect_learn_php.py not found"
fi
...
if [ -d "$EXT_DIR" ]; then
    ...
    NODE_OUT="build/Release/php_parser_addon.node"
    if [ -f "$NODE_OUT" ]; then
