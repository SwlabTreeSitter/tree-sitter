...
LANG_DIR="$ROOT/tree-sitter-ruby"
...
echo ""
echo "=== [2] Generate/Build (tree-sitter-ruby) ==="
cd "$LANG_DIR"
...
TOKEN_MAP_OUTPUT_DIR="$EXT_DIR/resources/ruby" "$TS_CLI" generate
"$TS_CLI" build

echo ""
echo "=== [3] Optional Parse Smoke Test ==="
# test/highlight/*.rb 는 tree-sitter annotation 전용 파일이므로 사용 불가
# 최소 Ruby 코드를 임시 파일로 생성하여 테스트
SAMPLE_TMP="$TS_DIR/_smoke_test_ruby.rb"
cat > "$SAMPLE_TMP" << 'EOF'
def hello(name)
  puts "Hello, #{name}!"
end
...
if "$TS_CLI" parse "$SAMPLE_TMP" --quiet 2>/dev/null; then
    ...
    echo " -> (warn) Parse test failed or segfaulted — skipping."
fi
rm -f "$SAMPLE_TMP"
...
if [ -f "to_data_batch_collect_learn_ruby.py" ]; then
    python3 to_data_batch_collect_learn_ruby.py
    python3 to_json_aggregate_ruby.py
else
    echo " -> (skip) to_data_batch_collect_learn_ruby.py not found"
fi
...
if [ -d "$EXT_DIR" ]; then
    ...
    NODE_OUT="build/Release/ruby_parser_addon.node"
    if [ -f "$NODE_OUT" ]; then
