...
LIB_PATH = "/home/hyeonjin/PL/tree-sitter-cpp/cpp.so"

SOURCE_DIR = "/home/hyeonjin/PL/codecompletion_benchmarks/cpp/TEST"
ANSWER_DIR = "/home/hyeonjin/PL/tree-sitter/reports/cpp"
REPORT_DIR = "/home/hyeonjin/PL/tree-sitter/reports/cpp"

DB_PATH = "/home/hyeonjin/PL/code-completion-extension/resources/cpp/candidates.json"

EXERCISM_PROJECT = "cpp-main"
...
def is_exercism_target(rel_path_unix: str) -> bool:
    ...
    if len(parts) >= 2:
        parent_dir = parts[-2]
        filename   = parts[-1]
        if parent_dir == ".meta" and filename in (
            "example.cpp", "exemplar.cpp",
        ):
            return True
    return False
...
class FileReporter:
    def __init__(self):
        ...
        if not os.path.exists(REPORT_DIR):
            ...
            for csv_file in ["cpp_file_performance.csv", "cpp_rank_distribution.csv"]:
    ...
    def run_cpp_at_position(self, target_file, byte_offset):
        cmd = [EXE_PATH, "cpp", LIB_PATH, target_file, "--byte", str(byte_offset), "2"]
        try:
    ...
    def save_debug_log(self, safe_name, log_data):
        debug_dir = os.path.join(REPORT_DIR, "debug_states_cpp")
    ...
    def save_file_performance_report(self):
        csv_path = os.path.join(REPORT_DIR, "cpp_file_performance.csv")
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(SOURCE_DIR):
            dirs[:] = [d for d in dirs if d not in {".git", "build", "vendor"}]
        ...
        print(f"[*] Found {len(target_files)} target C++ files. Starting Iterative Analysis...")
