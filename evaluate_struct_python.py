...
LIB_PATH = "/home/hyeonjin/PL/tree-sitter-python/python.so"

SOURCE_DIR = "/home/hyeonjin/PL/codecompletion_benchmarks/python/TEST"
ANSWER_DIR = "/home/hyeonjin/PL/tree-sitter/reports/python"
REPORT_DIR = "/home/hyeonjin/PL/tree-sitter/reports/python"

DB_PATH = "/home/hyeonjin/PL/code-completion-extension/resources/python/candidates.json"

EXERCISM_PROJECT = "python-main"
...
def is_exercism_target(rel_path_unix: str) -> bool:
    ...
    if len(parts) >= 2:
        parent_dir = parts[-2]
        filename   = parts[-1]
        if parent_dir == ".meta" and filename in ("example.py", "exemplar.py"):
            return True
    return False
...
class FileReporter:
    def __init__(self):
        ...
        if not os.path.exists(REPORT_DIR):
            ...
            for csv_file in ["python_file_performance.csv", "python_rank_distribution.csv"]:
    ...
    def save_debug_log(self, safe_name, log_data):
        debug_dir = os.path.join(REPORT_DIR, "debug_states_python")
    ...
    def save_file_performance_report(self):
        csv_path = os.path.join(REPORT_DIR, "python_file_performance.csv")
    ...
    def run(self):
        ...
        print(f"[*] Found {len(target_files)} target Python files. Starting Iterative Analysis...")
        ...
        if self.global_queries > 0:
            ...
            print(f"[Global] Python Fail      : {self.python_fail_count}")
