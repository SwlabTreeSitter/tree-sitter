...
LIB_PATH = "/home/hyeonjin/PL/tree-sitter-javascript/javascript.so"

SOURCE_DIR = "/home/hyeonjin/PL/codecompletion_benchmarks/javascript/TEST"
ANSWER_DIR = "/home/hyeonjin/PL/tree-sitter/reports/javascript"
REPORT_DIR = "/home/hyeonjin/PL/tree-sitter/reports/javascript"

DB_PATH = "/home/hyeonjin/PL/code-completion-extension/resources/javascript/candidates.json"

EXERCISM_PROJECT = "javascript-main"
...
def is_exercism_target(rel_path_unix: str) -> bool:
    """collect 스크립트와 동일한 필터"""
    parts = rel_path_unix.split("/")
    if len(parts) >= 2:
        parent_dir = parts[-2]
        filename   = parts[-1]
        if parent_dir == ".meta" and filename in ("proof.ci.js", "exemplar.js"):
            return True
    return False
...
class FileReporter:
    def __init__(self):
        ...
        if not os.path.exists(REPORT_DIR):
            ...
            for csv_file in ["javascript_file_performance.csv", "javascript_rank_distribution.csv"]:
    ...
    def run_cpp_at_position(self, target_file, byte_offset):
        cmd = [EXE_PATH, "javascript", LIB_PATH, target_file, "--byte", str(byte_offset), "2"]
        try:
    ...
    def save_debug_log(self, safe_name, log_data):
        debug_dir = os.path.join(REPORT_DIR, "debug_states_javascript")
    ...
    def save_file_performance_report(self):
        csv_path = os.path.join(REPORT_DIR, "javascript_file_performance.csv")
    ...
    def run(self):
        ...
        print(f"[*] Found {len(target_files)} target '.js' files. Starting Iterative Analysis...")
