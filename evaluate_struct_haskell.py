...
# 1. 실행 파일 및 라이브러리 경로 (Haskell 언어용)
...
class FileReporter:
    def __init__(self):
        ...
        if not os.path.exists(REPORT_DIR):
            ...
            for csv_file in ["haskell_file_performance.csv", "haskell_rank_distribution.csv"]:
    ...
    def save_debug_log(self, safe_name, log_data):
        debug_dir = os.path.join(REPORT_DIR, "debug_states_haskell")
    ...
    def save_file_performance_report(self):
        csv_path = os.path.join(REPORT_DIR, "haskell_file_performance.csv")
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(SOURCE_DIR):
            dirs[:] = [d for d in dirs if d not in {".git", "build", "dist", "dist-newstyle", ".stack-work"}]
            for file in files:
                if file.endswith(".hs"):
                    target_files.append(os.path.join(root, file))

        print(f"[*] Found {len(target_files)} '.hs' files. Starting Iterative Analysis...")
