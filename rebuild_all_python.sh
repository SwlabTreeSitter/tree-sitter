...
LANG_DIR="$ROOT/tree-sitter-python"
...
echo ""
echo "=== [2] Generate/Build (tree-sitter-python) ==="
cd "$LANG_DIR"
...
TOKEN_MAP_OUTPUT_DIR="$EXT_DIR/resources/python" "$TS_CLI" generate
"$TS_CLI" build

echo ""
echo "=== [3] Optional Parse Smoke Test ==="
SAMPLE_TMP="$TS_DIR/_smoke_test_python.py"
cat > "$SAMPLE_TMP" << 'EOF'
def add(a, b):
    return a + b
...
if [ -f "to_data_batch_collect_learn_python.py" ]; then
    python3 to_data_batch_collect_learn_python.py
    python3 to_json_aggregate_python.py
else
    echo " -> (skip) to_data_batch_collect_learn_python.py not found"
fi
...
if [ -d "$EXT_DIR" ]; then
    ...
    NODE_OUT="build/Release/python_parser_addon.node"
    if [ -f "$NODE_OUT" ]; then
